...
class Player {
  ...
  constructor(options) {
    ...
    if (!this.manager) this.manager = Utils_1.Structure.get("Player")._manager;
  ...
  connect() {
    ...
    if (!this.voiceChannelId) {
      throw new RangeError(
        "No voice channel has been set. You must use the `setVoiceChannelId()` method to set the voice channel before connecting."
      );
    }
  ...
  setAutoplay(autoplayState, botUser, tries) {
    ...
    if (autoplayState) {
      ...
      this.autoplayTries =
        tries && typeof tries === "number" && tries > 0 ? tries : 3; // Default to 3 if invalid
  ...
  async handleYouTubeRecommendations(track) {
      ...
      const searchResult = await this.manager.search(
        {
          ...
          source: Manager_1.SearchPlatform.YouTube,
        },
        track.requester
      );
    ...
    const res = await this.manager.search(
      { query: searchURI, source: Manager_1.SearchPlatform.YouTube },
      track.requester
    );
    if (
  ...
  async handlePlatformAutoplay(track, source, apiKey) {
      ...
      if (!title) {
        ...
        const randomTrack =
          response.data.toptracks.track[
            Math.floor(Math.random() * response.data.toptracks.track.length)
          ];
        const res = await this.manager.search(
          {
    ...
    if (response.data.error || !response.data.similartracks?.track?.length) {
      ...
      if (
        ...
        !retryResponse.data.toptracks?.track?.length
      )
        return [];
      const randomTrack =
        retryResponse.data.toptracks.track[
          Math.floor(Math.random() * retryResponse.data.toptracks.track.length)
        ];
      const res = await this.manager.search(
        {
          query: `${randomTrack.artist.name} - ${randomTrack.name}`,
  ...
  async setVolume(volume) {
    ...
    this.manager.emit(
      ...
      this,
      {
        ...
        details: {
          previousVolume: oldPlayer.volume || null,
   ...
   */
  async setSponsorBlock(
    segments = [
      Node_1.SponsorBlockSegment.Sponsor,
      Node_1.SponsorBlockSegment.SelfPromo,
    ]
  ) {
  ...
  async previous() {
    ...
    // Get the last played track and remove it from history
    const lastTrack = this.queue.previous.pop();

    // Move current track to the front of queue list for safety
    if (currentTrack) {
      this.queue.unshift(currentTrack); // Or you can use .add(currentTrack, 0) if you want debug events
    }

    // Prevent track end event from triggering next track
    this.set("skipFlag", true);

    // Play the previous track
    this.queue.current = lastTrack;
  ...
  async switchGuild(newOptions, force = false) {
    ...
    newOptions.selfDeafen =
      newOptions.selfDeafen ?? oldPlayerProperties.selfDeafen;
    ...
    clonedPlayer.dynamicRepeatIntervalMs =
      oldPlayerProperties.dynamicRepeatIntervalMs;
    ...
    this.manager.emit(
      ...
      `[PLAYER] Transferred player to a new server: ${JSON.stringify(
        debugInfo
      )}.`
    );
  ...
  async getCurrentLyrics(skipTrackSource = false) {
    ...
    const hasLyricsPlugin = this.node.info.plugins.some(
      (plugin) => plugin.name === "lavalyrics-plugin"
    );
    if (!hasLyricsPlugin) {
      throw new RangeError(
        `There is no lavalyrics-plugin available in the Lavalink node: ${this.node.options.identifier}`
      );
    }
